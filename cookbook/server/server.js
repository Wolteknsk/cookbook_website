  ...
  db.get('SELECT * FROM users WHERE email = ?', [email], (err, user) => {
    ...
    db.run('INSERT INTO users (email, password, name) VALUES (?, ?, ?)', 
      ...
      function(err) {
        ...
        res.json({ 
          user: { id: this.lastID, email, name: userName },
          token: 'token_' + this.lastID 
        });
      });
  });
});
...
app.post('/api/recipes', upload.fields([
  ...
  // Сохраняем путь к главному изображению
  let mainImagePath = null;
  if (req.files['mainImage']) {
    mainImagePath = '/uploads/' + req.files['mainImage'][0].filename;
  }
  
  // Сохраняем пути к изображениям шагов
  let stepImagesPaths = [];
  ...
  // Сохраняем инструкции с привязкой к изображениям
  const instructionsWithImages = instructionsArray.map((instruction, index) => ({
    text: instruction,
    image: stepImagesPaths[index] || null
  }));
  ...
  JSON.stringify(stepImagesPaths)   // ← должно быть
    ...
    res.status(201).json({ id: this.lastID, message: 'Рецепт создан' });
  });
});
